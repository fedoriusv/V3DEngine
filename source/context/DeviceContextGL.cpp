...
DeviceContextGL::DeviceContextGL(const platform::WindowPtr window)
...
{
#if defined(_PLATFORM_WIN_)
    ASSERT(!m_hRc, "m_hRc Already exits");
#endif //_PLATFORM_WIN_
}

bool DeviceContextGL::create()
{
#if defined(_PLATFORM_WIN_)
...
bool DeviceContextGL::present()
{
#if defined(_PLATFORM_WIN_)
    return SwapBuffers(m_hDc) != FALSE;
#else
    ASSER(false, "not implemented");
#endif //_PLATFORM_WIN_
}

#if defined(_PLATFORM_WIN_)
bool DeviceContextGL::createWinApiContext()
{
    LOG_INFO("Create WinApi GL Context");

    // Create a window to test antialiasing support
    HINSTANCE hInstance = GetModuleHandle(NULL);
    LPCWSTR className = __TEXT("V3DWinApiGL");
    ...
    if (newPixelFormat == 0 || !SetPixelFormat(m_hDc, newPixelFormat, &pfd))
    {
        LOG_WARNING("DriverContextGL::createWin32Context: Cannot create ExPixelFormat. Set default pixel format");
        newPixelFormat = ChoosePixelFormat(m_hDc, &pfd);
    ...
    DeviceContextGL::checkForErrors("Create Context");

    int pf = GetPixelFormat(m_hDc);
...
void DeviceContextGL::destroyWinApiContext()
{
    wglMakeCurrent(NULL, NULL);
    if (m_hRc)
    {
...
void DeviceContextGL::fillGrapthicCaps()
{
    ...
    m_graphicsCaps.setTextureUnitsCount(maxTextureUnits);
    ...
    m_graphicsCaps.setMaxAnisotropySize(maxAnisotropy);

    GLint samplesCount;
    glGetIntegerv(GL_SAMPLES, &samplesCount);
    m_graphicsCaps.setSamplesCount(samplesCount);

    GLint maxPatchVertices = 0;
    glGetIntegerv(GL_MAX_PATCH_VERTICES, &maxPatchVertices);
    m_graphicsCaps.setMaxPatchVertices(maxPatchVertices);
...
#ifdef _DEBUG_GL
    DeviceContextGL::printExtensionList();
#endif //_DEBUG_GL
}

void DeviceContextGL::checkForErrors(const std::string& location)
{
#ifdef _DEBUG_GL
...
bool DeviceContextGL::setVSync(bool use)
{
    bool succeed = false;
#if defined(_PLATFORM_WIN_)
...
void DeviceContextGL::printExtensionList() const
{
